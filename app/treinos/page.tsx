...
export default function Treinos() {
  ...
  const renderTreinoButton = (treino: ITreino, index: number) => {
    ...
    return (
      <div key={`treino-${treino.id}-${index}`} data-testid={`treino-button-${treino.id}`}>
        <Button
  ...
  return (
    <div className="flex min-h-screen flex-col bg-linear-to-br from-gray-900 to-black" data-testid="treinos-page">
      {/* Header com navegação */}
      <Header backRoute="/" />

      {/* Conteúdo principal */}
      <main className="flex flex-1 flex-col items-center justify-center px-8 py-16 text-center" data-testid="treinos-main">
        <Title>Escolha seu treino</Title>
        
        <div className="flex flex-col gap-6 sm:flex-row sm:gap-8" data-testid="treinos-container">
          {treinos.length >= 2 
